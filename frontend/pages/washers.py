...
st_autorefresh(interval=20000, key="washers_auto")
...
st.title("🧺 Washing Machines")
quote = get_random_quote()
st.markdown(f"> {quote}")

# Получаем и фильтруем только стиралки
machines = get_machines()
washers = [m for m in machines if m.get("type") == "washing"]

if not washers:
    st.info("No washing machines found.")
else:
    # Отображаем в виде сетки (3 колонки)
    cols = st.columns(3)
    for idx, machine in enumerate(washers):
        with cols[idx % 3]:
            with st.container(border=True):
                # Изображение
                st.image("https://cdn-icons-png.flaticon.com/512/2920/2920245.png", width=80)
