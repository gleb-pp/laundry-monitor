...
def test_busy_without_remaining_under_4_hours_is_busy(
    minutes_ago: int,
) -> None:
    """Check that a BUSY report without time under 4 hours is BUSY."""
    report = Report(
        machine_id=1,
    ...
    with patch("service.machine.datetime", FixedDateTime):
        assert (
            service._get_machine_status(machine) ==
            MachineResponseStatus.BUSY
        )


@given(minutes_ago=st.integers(min_value=240, max_value=10_000))
def test_busy_without_remaining_from_4_hours_is_probably_free(
    minutes_ago: int,
) -> None:
    """Check that a BUSY report without time from 4 hours is PROBABLY_FREE."""
    report = Report(
        machine_id=1,
        status=MachineReportStatus.BUSY,
        timestamp=NOW - timedelta(minutes=minutes_ago),
...
def test_busy_with_remaining_at_or_after_expiry_is_free(
    remaining: int,
    elapsed: int,
) -> None:
